from timecode_tools.generate_ltc import make_ltc
...
def create_gui():
    ...
    tk.Label(root, text="Stop Timecode (HH:MM:SS:FF):").grid(row=1, column=0, padx=10, pady=10)
    stop_timecode_entry = tk.Entry(root)
    stop_timecode_entry.insert(0, "24:00:00:00")  # Default stop timecode
    stop_timecode_entry.grid(row=1, column=1)

    tk.Label(root, text="Frames Per Second (FPS):").grid(row=2, column=0, padx=10, pady=10)
    fps_entry = tk.StringVar(root)
    fps_entry.set("30")
    fps_selector = tk.OptionMenu(root, fps_entry, "24", "25", "29.97DF", "30")
    fps_selector.grid(row=2, column=1)

    tk.Label(root, text="Output Directory:").grid(row=7, column=0, padx=10, pady=10)
    output_dir_entry = tk.Entry(root)
    output_dir_entry.grid(row=7, column=1)

    tk.Label(root, text="Offset (Minutes):").grid(row=4, column=0, padx=10, pady=10)
    offsets_entry = tk.Entry(root)
    offsets_entry.insert(0, "10")
    offsets_entry.grid(row=4, column=1)

    # Dropdown menu for Rate (44100 or 48000)
    tk.Label(root, text="Sample Rate (44100 or 48000):").grid(row=5, column=0, padx=10, pady=10)
    rate_var = tk.StringVar(root)
    rate_var.set("44100")  # Default value
    rate_menu = tk.OptionMenu(root, rate_var, "44100", "48000")
    rate_menu.grid(row=5, column=1)

    # Dropdown menu for Bits (8, 16, or 24)
    tk.Label(root, text="Bit Depth (8, 16, or 24):").grid(row=6, column=0, padx=10, pady=10)
    bits_var = tk.StringVar(root)
    bits_var.set("16")  # Default value
    bits_menu = tk.OptionMenu(root, bits_var, "8", "16", "24", '32', '64')
    bits_menu.grid(row=6, column=1)
    ...
    browse_button.grid(row=7, column=2, padx=10)

    # Add a progress bar widget
    progress_bar = ttk.Progressbar(root, orient="horizontal", length=300, mode="determinate")
    progress_bar.grid(row=11, column=0, columnspan=3, padx=10, pady=20)
    ...
    def start_generation():
        ...
        ##### Set Up Progress Bar 
        total_iterations = stop_minutes // offset
